...
float _Shininess;
float _MascotSpecularContributionCap;
sampler2D _MainTex;
sampler2D _ToonTex;
...
inline half4 LightingMMD (EditorSurfaceOutput s, half3 lightDir, half3 viewDir, half atten)
{
	...
	// Output
	float4 color = saturate( (_AmbColor * 0.25) + ( _Color * _LightColor0 ) );
	color *= s.Custom;
	float specularContributionCap = saturate(_MascotSpecularContributionCap);
	if (specularContributionCap <= 0.0)
	{
		specularContributionCap = 1.0;
	}
 	color += dirSpecular * specularContributionCap;
	color *= toon;
	color.rgb *= 0.90;
	color.a = s.Alpha;
	return color;
...
void surf (Input IN, inout EditorSurfaceOutput o)
{
	...
	o.Custom += sphereAdd * 0.20; // SphereAddTex Default:Black

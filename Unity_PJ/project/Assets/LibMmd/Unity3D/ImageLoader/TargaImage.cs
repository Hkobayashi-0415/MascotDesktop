...
namespace LibMMD.Unity3D.ImageLoader
{
	...
	public class TargaImage
	{
		...
		private void LoadTGAImage(BinaryReader binReader)
		{
			...
			#if LIBMMD_VERBOSE_TGA_LOGS
			UnityEngine.Debug.LogFormat("TGA LoadTGAImage: pixelDepth={0} format={1} dataLen={2}",
				this.objTargaHeader.PixelDepth, pf, bimagedata != null ? bimagedata.Length : 0);
			if (bimagedata != null && bimagedata.Length >= 4) {
				UnityEngine.Debug.LogFormat("TGA first 4 bytes: {0} {1} {2} {3}",
					bimagedata[0], bimagedata[1], bimagedata[2], bimagedata[3]);
			}
			#endif
			...
			if (pf == TextureFormat.BGRA32) {
				...
				var colors = Utils.Bgra32ToColors(bimagedata);
				#if LIBMMD_VERBOSE_TGA_LOGS
				if (colors != null && colors.Length > 0) {
					UnityEngine.Debug.LogFormat("TGA first color: R={0} G={1} B={2} A={3}", 
						colors[0].r, colors[0].g, colors[0].b, colors[0].a);
				}
				#endif
				bmpTargaImage.SetPixels (colors);
			} else if (pf == TextureFormat.RGB24) {
				// 24-bit TGA: BGR order needs to be converted to RGB
				bmpTargaImage = new Texture2D((int)this.objTargaHeader.Width, (int)this.objTargaHeader.Height);
				var colors = Utils.Bgr24ToColors(bimagedata);
				#if LIBMMD_VERBOSE_TGA_LOGS
				if (colors != null && colors.Length > 0) {
					UnityEngine.Debug.LogFormat("TGA 24-bit first color: R={0} G={1} B={2} A={3}", 
						colors[0].r, colors[0].g, colors[0].b, colors[0].a);
				}
				#endif
				bmpTargaImage.SetPixels (colors);

    ...
    public class MmdGameObject : MonoBehaviour
    {
        ...
        private void DoLoadModel(string filePath)
        {
            ...
            if (PhysicsMode == PhysicsModeEnum.Bullet)
            {
                ...
                {
                    PhysicsMode = PhysicsModeEnum.None;
                    Debug.LogWarningFormat("Bullet physics disabled because native plugin was not found: {0}", e.Message);
                }
                catch (Exception e)
                {
                    PhysicsMode = PhysicsModeEnum.None;
                    Debug.LogWarningFormat("Bullet physics disabled because initialization failed: {0}", e.Message);
                }
            }
            InitMesh();
        ...
        private void ResetMotionPlayer()
        {
            ...
            if (PhysicsMode == PhysicsModeEnum.Bullet && _physicsReactor != null)
            {
        ...
        private GameObject[] CreateRigids()
        {
            ...
            {
                result[i].GetComponent<Collider>().material = CreatePhysicsMaterial(_model.Rigidbodies, i);
            }
            return result;
        }
        ...
        private PhysicsMaterial CreatePhysicsMaterial(MmdRigidBody[] rigidbodys, uint index)
        {

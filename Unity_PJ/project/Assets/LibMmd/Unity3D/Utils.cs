...
namespace LibMMD.Unity3D
{
	public class Utils
	{
		...
		/// <summary>
		/// Convert BGR24 byte array to Color array (for 24-bit TGA)
		/// </summary>
		public static Color[] Bgr24ToColors(byte[] bytes) {
			var pixelCount = bytes.Length / 3;
			var ret = new Color[pixelCount];
			for (var i = 0; i < pixelCount; i++) {
				var offset = i * 3;
				ret [i] = new Color (
					bytes [offset + 2] / 255.0f, // R (from B position)
					bytes [offset + 1] / 255.0f, // G
					bytes [offset + 0] / 255.0f, // B (from R position)
					1.0f // A = 1 (opaque)
				);
			}
			return ret;
